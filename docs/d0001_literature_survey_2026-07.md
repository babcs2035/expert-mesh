...
> **【状態: 歴史的記録 — 対象は Iter1〜14】**
> 研究は Iteration 54 で収束した．**収束時点の現況は `docs/d0006_research_summary_iter28-54_2026-08.md`
> を参照すること**．本書の記述のうち，46 問・4 ノード・全ノード同一モデルという前提（第 1 部）は
> Iter15（1600 問化）・Iter17（教師あり分類器）・Iter18（domain LoRA）で覆っている．また §4.2 の
> 「10 分野化なら top1 81% 前後が現実的」という予測は外れた（実測 0.2059〜0.6056）．
> 文献の引用と当時の実測値の一次記録としては現在も有効である．
